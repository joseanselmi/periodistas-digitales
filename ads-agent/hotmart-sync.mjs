 ...
 *
 * USO:
 *   node hotmart-sync.mjs                 → ventas (histórico) + rechazos recientes
 *   node hotmart-sync.mjs --dry-run       → solo reporta, no escribe nada
 *   node hotmart-sync.mjs --since 2026-06-01   → ventas: solo desde esa fecha
 *   node hotmart-sync.mjs --rechazos-dias 7    → rechazos: ventana de captura (default 3)
 *   node hotmart-sync.mjs --no-rechazos        → salta la captura de rechazos (#36)
 *   node hotmart-sync.mjs --solo-rechazos      → solo captura rechazos (no toca ventas)
 *
