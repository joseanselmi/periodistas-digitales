...
en voseo, generada por `gen-agosto.mjs`).
...
`sistema-ingresos/campanas/muro/EMBUDO-GUIAS.md`.
...
node gen-muro-agosto.mjs

# 2. Slides 1080×1080 (Chromium). Una carpeta por día en para-subir/
node export-slides-auto.mjs carousels/muro-s1   # y muro-s2, muro-s3

# 3. Programar. Idempotente: lee la cola y saltea lo ya programado
node --env-file=.env.local schedule-muro.mjs
```
...
Por eso `schedule-muro.mjs` es idempotente: lee la cola primero y saltea las fechas
...
```bash
node gen-muro-stories.mjs                              # 16 HTML verticales
node export-stories.mjs                                # → JPG 1080×1920
node --env-file=.env.local post-story.mjs --dry-run    # dice cuál publicaría
node --env-file=.env.local post-story.mjs              # publica la de hoy
node --env-file=.env.local post-story.mjs 2026-08-17   # una fecha puntual
```

`post-story.mjs` es idempotente: deja constancia en
