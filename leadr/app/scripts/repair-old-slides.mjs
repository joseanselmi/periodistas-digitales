/**
 * Regenera los HTML de slides para las 13 clases que apuntan al proyecto viejo (INACTIVE).
 * Toma el slides_json del proyecto actual, genera el HTML y lo sube al bucket `slides` del proyecto actual.
 * Luego actualiza slides_url en la DB.
 *
 * Correr con: node scripts/repair-old-slides.mjs
 */
...
// ─── Generador de HTML (copiado de lib/slides-html.ts) ───────────────────────
...
function slideHtml(slide, brand, index, total) {
  ...
  switch (slide.type) {
    case 'title':
      content = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;text-align:center;padding:4rem;">
    ...
    case 'content':
      content = `<div style="padding:3.5rem 4rem;height:100%;box-sizing:border-box;">
    ...
    case 'bullets':
      content = `<div style="padding:3.5rem 4rem;height:100%;box-sizing:border-box;">
        ...
        <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:1rem;">
          ${(slide.bullets ?? []).map(b => `<li style="display:flex;align-items:flex-start;gap:1rem;font-size:1.05rem;color:${brand.text};opacity:0.9;line-height:1.5;"><span style="width:8px;height:8px;border-radius:50%;background:${brand.primary};margin-top:0.45rem;flex-shrink:0;"></span>${b}</li>`).join('')}
        </ul>
      </div>`; break
    ...
    case 'practice':
      content = `<div style="padding:3.5rem 4rem;height:100%;box-sizing:border-box;overflow:auto;">
        ...
        <ol style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:0.875rem;">
          ${(slide.steps ?? []).map((step, i) => `<li style="display:flex;align-items:flex-start;gap:1rem;font-size:1rem;color:${brand.text};opacity:0.9;line-height:1.5;"><span style="width:26px;height:26px;border-radius:8px;background:${brand.primary};color:${brand.bg};font-weight:700;font-size:0.8rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${i + 1}</span>${step}</li>`).join('')}
        </ol>
        ${slide.tip ? `<div style="margin-top:1.5rem;padding:1rem 1.25rem;background:${brand.primary}15;border:1px solid ${brand.primary}40;border-radius:10px;font-size:0.9rem;color:${brand.text};opacity:0.9;"><span style="color:${brand.primary};font-weight:600;">💡 Tip: </span>${slide.tip}</div>` : ''}
      </div>`; break
    ...
    case 'errors':
      content = `<div style="padding:3.5rem 4rem;height:100%;box-sizing:border-box;">
        ...
        <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:1rem;">
          ${(slide.bullets ?? []).map(b => `<li style="display:flex;align-items:flex-start;gap:1rem;font-size:1.05rem;color:${brand.text};opacity:0.9;line-height:1.5;"><span style="width:20px;height:20px;border-radius:50%;background:#EF444425;border:1.5px solid #EF4444;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:0.1rem;font-size:0.7rem;color:#EF4444;font-weight:700;">✕</span>${b}</li>`).join('')}
        </ul>
      </div>`; break

    case 'exercise':
      content = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;padding:3rem;text-align:center;">
        <div style="background:${brand.primary}10;border:1px solid ${brand.primary}35;border-radius:20px;padding:2.5rem 3rem;max-width:640px;width:100%;">
          ...
          ${slide.task ? `<div style="background:${brand.bg};border-radius:12px;padding:1.25rem 1.5rem;border:1px solid ${brand.primary}25;"><p style="color:${brand.primary};font-size:0.95rem;font-weight:500;line-height:1.6;">${slide.task}</p></div>` : ''}
        </div>
      </div>`; break

    case 'quote':
      content = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;padding:4rem;text-align:center;">
        <div style="font-size:4rem;color:${brand.primary};opacity:0.4;line-height:1;margin-bottom:1rem;">"</div>
        <blockquote style="font-size:1.5rem;color:${brand.text};line-height:1.6;max-width:700px;font-style:italic;margin:0 0 1.5rem;">${slide.quote ?? ''}</blockquote>
    ...
    case 'diagram': {
      ...
      content = `<div style="display:flex;flex-direction:column;height:100%;padding:2.25rem 3rem;gap:1rem;box-sizing:border-box;overflow:auto;">
        ...
        <div style="display:flex;flex-direction:column;align-items:center;gap:0.75rem;flex:1;justify-content:center;">
          <div style="background:${brand.primary};color:${brand.bg};padding:0.55rem 1.75rem;border-radius:12px;font-size:0.95rem;font-weight:700;text-align:center;box-shadow:0 0 20px ${brand.primary}44;flex-shrink:0;">${slide.center ?? ''}</div>
  ...
  return `<div class="slide" style="width:100%;height:100%;background:${brand.surface};border-radius:16px;overflow:hidden;position:relative;flex-direction:column;">
    ...
    <div style="position:absolute;bottom:1.5rem;right:2rem;font-size:0.75rem;color:${brand.text};opacity:0.3;font-family:monospace;">${index + 1} / ${total}</div>
    ${content}
  </div>`
}

function generateSlidesHTML(slides, brand = DEFAULT_BRAND) {
...
const { data: classes, error } = await supabase
  .from('classes')
  .select('id, title, slides_json, audio_urls')
  .like('slides_url', '%wbwfzsdhtbhkjlfuebrd%')
  .not('slides_json', 'is', null)

if (error) { console.error('Error fetching classes:', error); process.exit(1) }

console.log(`\n🔧 Reparando slides de ${classes.length} clases...\n`)

let ok = 0, fail = 0

for (const cls of classes) {
  const json = cls.slides_json
  const slides = Array.isArray(json) ? json : (json?.slides ?? [])

  if (!slides.length) {
    console.log(`  ⚠️  Clase ${cls.id} "${cls.title}" — slides_json vacío, saltando`)
    fail++
    continue
  }

  const html = generateSlidesHTML(slides)
  const path = `classes/${cls.id}-${Date.now()}/index.html`
  ...
  if (uploadErr) {
    console.error(`  ❌ Clase ${cls.id} — error upload:`, uploadErr.message)
    fail++
    continue
  }
  ...
  if (updateErr) {
    console.error(`  ❌ Clase ${cls.id} — error update DB:`, updateErr.message)
    fail++
    continue
  }

  console.log(`  ✅ Clase ${cls.id} "${cls.title}"`)
  ok++
}

console.log(`\n✅ ${ok} slides reparadas | ❌ ${fail} fallidas\n`)
